...
// @Component({
//   selector: 'app-tstool-config',
//   templateUrl: './tstool-config.component.html',
//   styleUrls: ['./tstool-config.component.css']
// })
// export class TstoolConfigComponent implements OnInit {
...
export class TstoolConfigComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.eventEmitterService.subsVar==undefined) {  
      ...
      invokeTSToolHCComponentFunction.subscribe((name:string) => {    
        // this.openDialog();    
        this.graphTsToolConfig();
      });    
    } 
  ...
  /**
             ...
             */
  replaceProperties(graphTemplateObject: Object,
  ...
  //  let path = 'assets/TsToolGraphConfigFiles/diversion-graph-template.json';
 ...
 public getJSONData(path: string): Observable<any> {    
  ...
  graphTsToolConfig(){
      this.getJSONData('assets/app/TsToolGraphConfigFiles/county-population-graph-config.json').subscribe((graphTemplateObject: Object) => {
...
// ----------------------------------------------------------------------------------------------------
  @Component({
    selector: 'ts-dialog-content',
    templateUrl: './dialog-content.html'
  })
  export class TSDialogContent {
    ...
    constructor(public dialogRef: MatDialogRef<TSDialogContent>,
          ...
          tooltips: {
            callbacks: {
              title: function (tooltipItem, data) {
                ...
                return tooltipItem[0].xLabel.toString();
              }
            }
          },
  ...
  private createCSVChartJSGraph(results: any): void {
    ...
    createTSGraph(timeSeries: any[]): void {    
      ...
      // this.createGraph(configArray);
        ...
        if (!this.chartPackage) {
          ...
          chartJSGraph = false;
        } else if (this.chartPackage.toUpperCase() === 'HIGHCHARTS') {
          chartJSGraph = false;
        } else {
          chartJSGraph = true;
        }
    ...
    private createHighchartsGraph(config: PopulateGraph[],  CSV:boolean): void {
      ...
      Highcharts.chart('container', {
        title: {
          text: 'Larimer County Total Population'
      },
    ...
    public setTSIDLocation(TSIDLocation){
    ...
    public getTSIDLocation(){
      return this.TSID_Location;
    }
    private handleError<T> (path: string, type?: string, result?: T) {
      return (error: any): Observable<T> => {
        // Log the error to console instead
  ...
  public buildPath(pathType: string, arg?: any[]): string {
    let appPath = "assets/app/";

    switch(pathType) {
      // case 'contentPagePath':
      //   return  appPath + this.getContentPathFromId(arg[0]);
      // case 'geoLayerGeoJsonPath':
      //   return appPath + this.getGeoJSONBasePath() + arg[0];
      // case 'homePagePath':
      //   return appPath + this.getHomePage();
      // case 'popupConfigPath':
      //   return appPath + this.getMapConfigPath() + arg[0];
      case 'csvPath':
      case 'docPath':
      case 'symbolImage':
      case 'stateModPath':
      ...
      case 'classificationPath':
        return appPath + this.formatPath(arg[0], pathType);
  ...
  export class StateMod_TS {
    ...
    */
    ...
    readTimeSeries(tsident_string: string, filename: string, date1?: any, date2?: any,
      ...
      return this.getPlainText(filename, 'StateMod Data File').pipe(map((stateModFile: any) => {
